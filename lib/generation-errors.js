...
const BUSY =
  "Sitebric is at capacity right now. Nothing was charged against your plan — please try again in a few minutes.";
...
export function friendlyGenerationError(err) {
  ...
  if (/^multipage_/.test(raw)) {
    return "This four-page site didn't come together cleanly. Nothing was charged against your plan — please try again, or generate it as a single-page site.";
  }
  if (err?.name === "AbortError" || /abort/i.test(raw)) {
    return "This site took longer than the time limit allows. Nothing was charged against your plan — try a shorter brief, or generate it as a single-page site.";
  }
  ...
  if (/timeout|timed out|ECONNRESET|socket hang up|fetch failed|aborted/i.test(raw)) {
    return "The connection dropped while this site was being built. Nothing was charged against your plan — please try again.";
  }

  return "Something went wrong while building this site. Nothing was charged against your plan — please try again.";
}
