...
// True when the failure is our own account being out of credit or over
// quota, rather than anything the reseller did or a transient blip.
//
// Exported so the alerting path and the message path read the same
// condition — if these two ever disagree, either the owner is told about
// outages that aren't happening, or isn't told about the one that is.
export function isBillingFailure(err) {
  const raw = String(err?.message || "");
  return /credit balance|billing|insufficient|quota|payment/i.test(raw);
}
...
export function friendlyGenerationError(err) {
  ...
  // is working fine.
  if (isBillingFailure(err)) return BUSY;
