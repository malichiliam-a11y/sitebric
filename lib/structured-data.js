...
// Monthly price in USD, matching the pricing page.
...
export function softwareApplicationSchema() {
  return {
    ...
    offers: {
      "@type": "AggregateOffer",
      priceCurrency: "USD",
      lowPrice: String(Math.min(...PLAN_ORDER.map((p) => PLAN_PRICES[p]))),
      highPrice: String(Math.max(...PLAN_ORDER.map((p) => PLAN_PRICES[p]))),
      offerCount: PLAN_ORDER.length,
      offers: PLAN_ORDER.map(offer),
    },
  };
}
