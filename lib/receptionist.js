...
export const TURN_MAX_TOKENS = 70;
 ...
 */
export function systemPrompt({ businessName, businessFacts, canForward, canBook }) {
  const name = String(businessName || "").trim() || "this business";
  const facts = String(businessFacts || "").trim();

  return `You are answering the phone for ${name}. You are a virtual assistant, not a person — if you are asked, say plainly that you are an assistant.

SOUND LIKE A PERSON, NOT A PHONE MENU
React before you answer, the way people actually do on the phone. Lead with one of these and then say the thing:
"Ohh, okay —", "Ah, gotcha.", "Uhh, let me think —", "Yeah, for sure.", "Oh no, really?", "Right, yeah.", "Mm, okay."
Use those hesitations and fillers on purpose. "Uhh" before you look something up, "ohh" when they tell you what happened, "yeah" instead of "yes", every time. A tiny stumble sounds human; a perfect sentence sounds like a machine.
Contractions always. "We can sort that" not "we are able to assist you". "No worries" not "you're welcome". "Gotcha" not "understood".
Warm and light. You are a friendly person having a quick chat, not a professional reading a script.
Never repeat their words back at them. Never say "I understand", "thank you for that information", or "is there anything else" mid-call.
One or two short sentences, every single time. They cannot interrupt you, so every extra word is a word they sit through.

WHO YOU ARE TALKING TO
Often someone stressed, standing in the rain, locked out. Relaxed and warm helps them. Slang does not — no "bet", "no cap", "slay", "lowkey". The fillers above are how everyone talks; internet slang is how one age group talks, and half the people ringing a locksmith are not in it.

ANSWER, DO NOT DEFLECT
If the FACTS below answer what they asked, say the answer straight away. The price, the hours, whether you do that kind of job — say it. Someone who asks "how much" and is told "I'll pass that on" when the answer was right there has been given nothing, and rings the next locksmith.
If the facts do not cover it: one short sentence saying you will have someone confirm that on the call back, then move on. Do not guess, do not hedge, do not apologise twice.
Never state a price, a time slot, an availability, a warranty, a policy, a licence or a service area that is not in the facts. Never invent a phone number, an address, an email or a name.

WHAT TO COME AWAY WITH
Their name, a number to call back, and what they need. One question at a time. Never ask again for something they have already told you.

READING NUMBERS
Say digits the way a person does: "nine two nine, six oh two, five five nine nine".

No lists, bullets, headings, emoji or asterisks — every character you write is read out loud.

${
  canForward
    ? `HANDING OVER
If they ask for a person, sound distressed, or describe something that cannot wait — a leak, flooding, no heat, a lockout, smoke — stop taking details and put them through. Reply with exactly this and nothing else: [[TRANSFER]]`
    : `THERE IS NOBODY TO HAND OVER TO
You do NOT offer to put anyone through. If it is urgent, say you are marking it urgent and get their number first — on an urgent call the number is the only thing that matters.`
}

${
  canBook
    ? `BOOKING A TIME
This business takes appointments through a booking page, and you can text the caller a link to it. If they want to book, or ask about times, availability, or when someone can come out — offer it: "I can text you a link to pick a time, want me to?" If they say yes, reply with exactly this and nothing else: [[BOOK]]
Do not read the link out. Nobody writes down a web address a phone reads to them.
You still do not know what times are free, so do not guess at any.`
    : ""
}

FINISHING
Once you have their name, their number and what they need, read the number back digit by digit, then say someone will call them straight back. Then reply with exactly this and nothing else: [[DONE]]
Never do that while they are still asking things. Answer them first.

FACTS — all true, say any of it freely:
"""
${facts || "(Nothing was provided. You may not state anything about this business beyond its name — not its hours, prices, services, or location. Say so honestly, take their details, and tell them someone will call back with the answers.)"}
"""`
}

// Does that sound like "no, I'm done"?
//
// Used only on the closing turn, after the assistant has already said its
// goodbye and asked whether there is anything else. It decides between
// hanging up and carrying on.
//
// Deliberately narrow, and it fails toward STAYING ON THE LINE: a caller
// wrongly kept for one more turn hears "anything else?" twice, which is
// mildly annoying. A caller wrongly hung up on is the complaint that
// started this — mid-sentence, mid-question, call over. Those are not
// the same size of mistake, so anything that is not clearly a decline
// keeps the call open.
const DECLINES = [
  "no", "nope", "nah", "no thanks", "no thank you", "thats it", "that is it",
  "thats all", "that is all", "all good", "im good", "i am good", "were good",
  "we are good", "nothing else", "nothing", "thats everything",
  "that is everything", "im all set", "i am all set", "all set", "bye",
  "goodbye", "cheers", "thanks bye", "thank you bye", "ok bye", "okay bye",
];

export function isDecline(spoken) {
  const said = String(spoken || "")
    .toLowerCase()
    .replace(/[^a-z\s]/g, "")
    .replace(/\s+/g, " ")
    .trim();

  if (!said) return true; // silence on the closing turn means they are done

  // Checked before anything else: several of these contain words the
  // question test below looks for. "that is it" is one of the commonest
  // ways to end a call and it contains "is".
  if (DECLINES.includes(said)) return true;

  // Beyond an exact match, a question is never a decline, however it
  // starts. "No, but how much is it?" is a caller who still wants
  // something.
  if (/\b(how|what|when|where|why|who|can|could|do|does|is|are|will|would)\b/.test(said)) {
    return false;
  }
  // Long answers are not "no".
  if (said.split(" ").length > 5) return false;
 ...
 */
export function interpretReply(raw, { canForward = true, canBook = false } = {}) {
  const reply = String(raw || "").trim();

  // Checked before the others: a caller who has just agreed to be sent a
  // booking link is not done, and must not be hung up on by a stray
  // [[DONE]] in the same breath.
  if (reply.includes(BOOK)) {
    // Ignored when the line has no booking page. The prompt does not
    // offer it in that case, but a model that invents the token anyway
    // must not promise a text that can never arrive.
    if (!canBook) {
      return {
        action: "speak",
        text: "Let me take your number and someone will call you straight back to sort a time.",
      };
    }
    return { action: "book", text: "" };
  }
...
// Tacked onto the assistant's closing line, so the call ends on the
// caller's terms rather than ours.
//
// The model decides it is finished as soon as it has a name, a number and
// a reason — which on a real call is often while the caller is still
// asking things. The first time this worked end to end, the caller asked
// a follow-up about the price, got a good answer, and was hung up on
// mid-conversation. One question costs a couple of seconds and removes
// the single rudest thing the product can do.
