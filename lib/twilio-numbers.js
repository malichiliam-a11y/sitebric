...
export async function buyNumber({ phoneNumber, baseUrl }) {
  ...
  const body = new URLSearchParams({
    ...
    // Stated rather than left to a default. The routes answer both
    // methods now, so a number configured either way still works — but
    // every method left unstated here is one more thing that can differ
    // between a number bought today and one bought a year ago.
