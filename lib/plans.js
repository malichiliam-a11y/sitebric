...
// KILL SWITCH — off until multi-page fits inside the platform's function
// time limit.
//
// A four-page build takes roughly five minutes of model streaming and
// /api/generate is capped at 300 seconds, so the function was killed with
// the site already written and still in flight. The output is billed by
// the model API the moment it's generated, so every attempt cost real
// money and delivered nothing — twice, before this went off.
//
// Turning this back on requires the generation to be split across
// parallel calls (design system + home first, then the other three pages
// concurrently), not just a longer timeout.
