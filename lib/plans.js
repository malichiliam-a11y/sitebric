...
export const PLAN_LIMITS = {
  none: { sites: 0, generations: 0, searches: 0, customDomains: false, receptionist: false, label: "No plan" },
  trial: { sites: 2, generations: 2, searches: 2, customDomains: false, receptionist: false, label: "Free Trial" },
  starter: { sites: 5, generations: 10, searches: 20, customDomains: false, receptionist: false, label: "Starter" },
  growth: { sites: 20, generations: 40, searches: 50, customDomains: true, receptionist: true, label: "Growth" },
  pro: { sites: 100, generations: 150, searches: 150, customDomains: true, receptionist: true, label: "Pro" },
};
...
// The AI receptionist — a phone number that answers when the business
// can't — is Growth and Pro.
//
// Not a marketing decision. Every number carries a monthly Twilio charge
// whether or not it ever rings, plus per-minute cost on every call, so it
// is the first thing in this product with a real recurring cost per
// customer. A trial account with a phone number is a bill with no revenue
// behind it.
export function canUseReceptionist(plan) {
  return Boolean(limitsFor(plan).receptionist);
}
