...
(function () {
  ...
  function show(id) {
    ...
    // Found by class, never by getElementById. A page body can carry an
    // id matching a page name — a services teaser on the home page, a
    // contact CTA on every page — and those sit earlier in the document
    // than the real page containers, so getElementById returned the
    // teaser and the actual page never became active. The site then
    // showed nothing but the nav and the footer.
    var pages = document.querySelectorAll('.page');
    var target = null;
    for (var i = 0; i < pages.length; i++) {
      var isIt = pages[i].id === id;
      pages[i].classList.toggle('active', isIt);
      if (isIt) target = pages[i];
    }
  ...
  function currentId() {
    try { return (location.hash || '').replace('#', ''); } catch (err) { return ''; }
  }
  function route() { show(currentId()); }

  // Page links are handled here rather than by letting the browser follow
  // them. Inside a srcdoc preview iframe the document's base URL is
  // inherited from the parent page, so an href="#services" resolves
  // against the dashboard's own URL — following it loads the dashboard
  // into the preview frame, where the sandbox blocks its scripts and the
  // visitor gets an application error instead of the site. Intercepting
  // the click keeps navigation entirely in-page, and the same handler
  // works unchanged on the published site.
  document.addEventListener('click', function (e) {
    if (!e.target.closest) return;

    var toggle = e.target.closest('[data-nav-toggle]');
    if (toggle) { e.preventDefault(); document.body.classList.toggle('nav-open'); return; }
    ...
    // Anything that genuinely leaves the site is left alone: real
    // destinations, phone numbers, email, downloads.
    // Deliberately two checks rather than one regex: a '//' alternative
    // inside a regex literal closes the literal early, which throws a
    // SyntaxError and takes the whole router down with it.
    if (/^[a-z][a-z0-9+.-]*:/i.test(href)) return;
    if (href.charAt(0) === '/' && href.charAt(1) === '/') return;

    // Everything else is internal, and internal navigation must never be
    // left to the browser. Inside a preview frame the document's base URL
    // is the dashboard's, so following an internal link loads the
    // dashboard into the frame. That happened twice: first via hash links,
    // then via "index.html" links in a nav the model duplicated into a
    // page body.
    e.preventDefault();

    var id = href.charAt(0) === '#'
      ? href.slice(1)
      // "properties.html", "./about.html#team" -> "properties", "about"
      : href.split('#')[0].split('/').pop().replace(/\.html?$/i, '');
    if (!id) return;

    // Names the model reaches for when it invents an extra nav entry.
    // Mapping them beats dropping the visitor on the home page, which is
    // what an unrecognised name would otherwise do.
    var ALIASES = {
      index: 'home', main: 'home', landing: 'home',
      properties: 'services', listings: 'services', portfolio: 'services',
      gallery: 'services', work: 'services', pricing: 'services',
      plans: 'services', buy: 'services', sell: 'services', shop: 'services',
      team: 'about', story: 'about', 'our-story': 'about', company: 'about',
      quote: 'contact', booking: 'contact', book: 'contact',
      'get-quote': 'contact', enquire: 'contact', inquiry: 'contact'
    };
    ...
    // Some other anchor: find it, switch to whichever page contains it,
    // then scroll to it. This is what makes an invented nav entry like
    // "Properties" land somewhere real instead of doing nothing.
    var el = null;
    try { el = document.getElementById(id); } catch (err) {}
    if (!el) return;

    var owner = el.closest ? el.closest('.page') : null;
    if (owner && !owner.classList.contains('active')) {
      try {
        if (location.hash !== '#' + owner.id) history.pushState(null, '', '#' + owner.id);
      } catch (err) {}
      show(owner.id);
    }
...
<<<NAV>>>
The sticky nav markup, which appears on every page. It must contain the business name, EXACTLY FOUR page links — Home, Services, About, Contact — and a "Call Now" / "Get a Quote" button (see CONTACT below for what that links to).

Do NOT add extra nav entries. There are four pages and only four. Inventing a fifth or sixth entry ("Properties", "Buy", "Sell", "Gallery", "Pricing") produces a nav link with no page behind it, which is a dead link on the finished site. If this business has areas that deserve their own emphasis, they belong as sections INSIDE the Services page, not as nav entries. You may rename the four links to suit the business — a real estate firm might use "Listings" instead of "Services" — but the href must stay exactly #home, #services, #about and #contact, and there must be four of them. Every page link must be written exactly as <a href="#home" data-nav="home">, <a href="#services" data-nav="services">, <a href="#about" data-nav="about">, <a href="#contact" data-nav="contact"> — the data-nav attribute is what marks the current page as active. A mobile menu button must carry the attribute data-nav-toggle. Do not write any JavaScript.
...
${name === "home" || name === "contact" ? orderBlock : ""}
...
Do NOT include a nav or a footer. Both already exist and are added around your output automatically — a page that brings its own ends up with two of each stacked on top of one another. Start at this page's first content section and stop at its last.

Never link to a file. There are no separate files: the whole site is one document and the four pages are reached by href="#home", href="#services", href="#about" and href="#contact". A link to "index.html" or "properties.html" points at nothing and breaks.
...
// A page body must be page content and nothing else. Asked for one page,
// the model returned a whole document instead — its own <nav> linking to
// index.html and properties.html, plus a footer — which then sat inside
// the real nav and footer the shell already supplies. Those .html links
// are not hash links, so the router left them to the browser, they
// resolved against the dashboard's URL inside the preview frame, and the
// dashboard loaded into it.
//
// Applied at assembly rather than only at generation, so re-stitching a
// site already stored with duplicated chrome cleans it without
// regenerating anything.
function stripPageChrome(html) {
  return stripStrays(html)
    .replace(/<!DOCTYPE[^>]*>/gi, "")
    // The lookahead is essential: without it "head" also matches the
    // start of "<header>", so every <header> and </header> in a page body
    // was deleted. That destroyed the styled header band at the top of
    // each page, leaving unstyled text and content bunched against the
    // footer — pages that looked broken and flat rather than designed.
...
// A page body routinely carries an id matching one of the four page
// containers — a services teaser on the home page, a contact CTA on every
// page. Duplicate ids are invalid HTML and make any lookup by id
// ambiguous, so the inner one is renamed. Links to "#contact" still work:
// the router resolves those to the contact page rather than the anchor.
function dedupePageIds(html, pageId) {
  return String(html).replace(
    /(\sid=)(["'])(home|services|about|contact)\2/gi,
    (_m, attr, quote, id) => `${attr}${quote}${pageId}-${id.toLowerCase()}${quote}`
  );
}

// A nav that lists more entries than there are pages sends several of
// them to the same place — Properties, Buy and Sell all landing on
// Services reads as broken navigation even though every link works.
//
// Redundant entries are dropped, keeping the first link to each page.
// A link carrying data-nav is always kept: those are the real nav items,
// and the one duplicate worth protecting is the brand, which points at
// home, comes first, and is therefore never the duplicate anyway.
function dedupeNavLinks(navHtml) {
  const seen = new Set();
  return String(navHtml).replace(/<a\b[^>]*>[\s\S]*?<\/a>/gi, (anchor) => {
    const href = (anchor.match(/href=["']([^"']*)["']/i) || [])[1] || "";
    if (href.charAt(0) !== "#") return anchor;
    const id = href.slice(1).toLowerCase();
    if (!PAGES.includes(id)) return anchor;
    if (/\sdata-nav\s*=/i.test(anchor)) { seen.add(id); return anchor; }
...
function assemble({ head, css, nav, footer, bodies, clientName }) {
  const pageSections = PAGES.map(
    (p) => `<section class="page" id="${p}">\n${dedupePageIds(stripPageChrome(bodies[p]), p)}\n</section>`
  ).join("\n");
...
<body>
${dedupeNavLinks(stripStrays(nav))}
${pageSections}
${stripStrays(footer)}
<script>
${ROUTER_SCRIPT}
