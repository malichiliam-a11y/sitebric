...
- Never write <script> tags, IntersectionObserver, or scroll listeners. They will be stripped.`;
  ...
  const shell = await callModel(anthropic, { prompt: shellPrompt, maxTokens: 26000, signal });
...
Use class="reveal" on elements that should animate in, and <span class="counter" data-target="...">0</span> for counting statistics. Never write JavaScript — it is provided and yours would be stripped.`;
  ...
  const [home, services, about, contact] = await Promise.all(
    PAGES.map(async (name) => {
      const r = await callModel(anthropic, { prompt: pagePrompt(name), maxTokens: 14000, signal });
      // Same guard as the shell: a page cut off mid-tag is broken markup,
      // and shipping it would be worse than failing.
      if (r.stopReason === "max_tokens") throw new Error(`multipage_page_truncated_${name}`);
      return stripStrays(r.text);
    })
  );

  const bodies = { home, services, about, contact };
