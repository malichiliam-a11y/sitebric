...
// nothing.
//
// So the work is split. One call produces the shared shell — head, CSS,
// nav, footer — plus the home page. The other three pages are then
// generated concurrently against that CSS, so total wall time is about
// two page-times instead of four.
//
...
export async function generateMultiPageSite({
  ...
  // ---- Phase 1: shared shell + home page ----
  const shellPrompt = `You are building a FOUR-PAGE website for a small local business, as one HTML file. This first request produces the shared shell and the home page only. The other three pages are generated separately against the CSS you write here, so the CSS must cover them too.
...
<<<HOME>>>
The INNER markup of the home page only — no <section class="page"> wrapper, that is added for you. Contents: a hero with a specific headline, supporting subheadline, call-to-action and hero visual; a teaser of 3 core services as cards each linking to href="#services"; a "why choose us" block with 3-4 concrete differentiators; 2-3 short realistic testimonials with names; and a final call-to-action linking to href="#contact".
...
- Never write <script> tags, IntersectionObserver, or scroll listeners. They will be stripped.`;
  ...
  const shell = await callModel(anthropic, { prompt: shellPrompt, maxTokens: 20000, signal });
  if (shell.stopReason === "max_tokens") {
    throw new Error("multipage_shell_truncated");
  }

  const head = section(shell.text, "HEAD");
  const css = section(shell.text, "CSS");
  const nav = section(shell.text, "NAV");
  const footer = section(shell.text, "FOOTER");
  const home = section(shell.text, "HOME");

  if (!css || !nav || !home) {
    throw new Error("multipage_shell_unparseable");
  }

  // ---- Phase 2: the other three pages, concurrently ----
  // Concurrent is the whole point: three sequential page calls would put
  // wall time back over the function's ceiling.
  const pagePrompt = (name) => `You are writing ONE page of an existing four-page website for "${clientName}". The site's design and stylesheet already exist — reuse its classes exactly and invent no new ones unless you must, since no new CSS can be added.
...
Use class="reveal" on elements that should animate in, and <span class="counter" data-target="...">0</span> for counting statistics. Never write JavaScript — it is provided and yours would be stripped.`;
  ...
  const [services, about, contact] = await Promise.all(
    ["services", "about", "contact"].map(async (name) => {
      const r = await callModel(anthropic, { prompt: pagePrompt(name), maxTokens: 12000, signal });
      return stripStrays(r.text);
    })
  );

  const bodies = { home: stripStrays(home), services, about, contact };
  for (const p of PAGES) {
    if (!bodies[p]) throw new Error(`multipage_page_empty_${p}`);
  }

  return assemble({ head, css, nav, footer, bodies, clientName });
}
